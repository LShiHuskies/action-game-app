...
const HealthBar = ({ ammoIcon, IMG, ammoLeft, healthBar, selfUserGame, userGame }) => {
  ...
  const addlStyle = (!selfUserGame || !userGame) || userGame.user_id === selfUserGame.user_id ? { position: 'absolute', top: '0' } : {};
    ...
    return (
        <div id="top-health" style={style}>
          <div class="heath-row" style={{ ...style, ...addlStyle }}>
            <img src={Health.HealthIcon.src} class="health-heart" />
            <div className="health">
              <span style={{width: `${healthBar}%`}}>{healthBar}% </span>
            </div>
          </div>
          { !userGame || !selfUserGame || selfUserGame.id === userGame.id ?
